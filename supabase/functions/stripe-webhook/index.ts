...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!signature) {
      return new Response(JSON.stringify({ error: "Sin firma de Stripe" }), {
    ...
    if (event.type === "checkout.session.completed") {
      ...
      const minecraft_nick = session.metadata?.minecraft_nick;
      const server = session.metadata?.server || 'mods';

      if (!orderId || !minecraft_nick) {
        console.error("Metadata incompleta en sesión:", session.id);
      ...
      // ✅ Enviar comandos RCON al servidor
      ...
      for (const item of items) {
        ...
        if (rankCmd) {
          const cmd = rankCmd.replace('{nick}', minecraft_nick);
          const ok = await sendRconCommand(cmd);
          if (!ok) rconSuccess = false;
        }

        if (chunkCmd) {
          const cmd = chunkCmd.replace('{nick}', minecraft_nick);
          const ok = await sendRconCommand(cmd);
          if (!ok) rconSuccess = false;
        }
      }
      ...
      if (!rconSuccess) {
        console.warn(`Algunos comandos RCON fallaron para ${minecraft_nick} — revisar manualmente`);
      }
